...
function parseCSV(t){
  ...
  return rows.map(a=>{
    const o={};
    headers.forEach((h,i)=>o[h]=clean(a[i]));
    return o;
  });
}
function norm(s){return String(s??"").normalize("NFKC").replace(/\u00A0/g," ").trim().replace(/\s+/g," ").toLowerCase();}
const TYPE_MAP=Object.fromEntries(TYPES.map(x=>[norm(x),x]));
function canonicalType(v){return TYPE_MAP[norm(v)]||"";}
function num(v){
  let s=String(v??"").replace(/\s/g,"").replace(",",".");
  let m=s.match(/-?\d+(?:\.\d+)?/);
  return m?Number(m[0]):null;
}
const VOLTAGE_SPLIT_TYPES=[
  "Power Transformer",
  "Current Transformer",
...
const ALL_VOLTAGE_TYPES=[
  "Neutral Grounding Resistance",
  "Reactor",
  "Capacitor"
];
function voltageGroup(v){
  const s=norm(v).replace(/\s+/g,"");
  if(s==="150kv" || s==="500kv") return "transmisi";
  return "incoming";
}
function includeByVoltageGroup(r, group){
  const t=canonicalType(r["Jenis Aset"]);
  if(ALL_VOLTAGE_TYPES.includes(t)) return true;
  if(VOLTAGE_SPLIT_TYPES.includes(t)) return voltageGroup(r.Tegangan)===group;
  return false;
}
...
function fill(id,values,selected=""){
 let s=$(id);s.innerHTML='<option value="">Semua</option>'+values.map(v=>`<option>${v}</option>`).join("");
 if(values.includes(selected))s.value=selected;
}
function hierarchyRows(exclude){
 const f={unitInduk:$("unitInduk").value,upt:$("upt").value,ultg:$("ultg").value,gi:$("gi").value,bay:$("bay").value};
 return raw.filter(r=>{
   if(!includeByVoltageGroup(r,$("voltageGroup").value))return false;
   if(exclude!=="unitInduk"&&f.unitInduk&&r["Unit Induk"]!==f.unitInduk)return false;
   if(exclude!=="upt"&&f.upt&&r["Unit Pelaksana"]!==f.upt)return false;
   if(exclude!=="ultg"&&f.ultg&&r.Ultg!==f.ultg)return false;
...
function refreshCascading(){
 const keep={unitInduk:$("unitInduk").value,upt:$("upt").value,ultg:$("ultg").value,gi:$("gi").value,bay:$("bay").value};
 fill("unitInduk",unique("Unit Induk",hierarchyRows("unitInduk")),keep.unitInduk);
 fill("upt",unique("Unit Pelaksana",hierarchyRows("upt")),keep.upt);
 fill("ultg",unique("Ultg",hierarchyRows("ultg")),keep.ultg);
 fill("gi",unique("Gardu Induk",hierarchyRows("gi")),keep.gi);
 fill("bay",unique("Bay",hierarchyRows("bay")),keep.bay);
}
...
function ahiDonut(){donut("ahiDonut",AHI.map(([n])=>({n,v:filtered.filter(r=>ahiNorm(r.AHI)===ahiNorm(n)).length})),filtered.filter(r=>/^([1-5]) - /.test(r.AHI)).length)}
function assetAhiVisual(){
  const colors=["#20a46b","#f2b400","#f07800","#e53935","#111"];
  const rows=TYPES.map(t=>{
    const a=filtered.filter(r=>canonicalType(r["Jenis Aset"])===t);
    const total=a.length||0;
    const vals=AHI.map(([n])=>a.filter(r=>ahiNorm(r.AHI)===ahiNorm(n)).length);
